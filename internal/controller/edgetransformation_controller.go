...
// EdgeTransformationReconciler reconciles an EdgeTransformation object.
type EdgeTransformationReconciler struct {
	...
	// nginx config as an upstream target. May be empty when widget
	// injection is not used; rendering still succeeds, the upstream just
	// resolves to a placeholder that won't be hit.
	OperatorAPIAddr string
}

// +kubebuilder:rbac:groups=route-prism.egoavara.net,resources=edgetransformations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route-prism.egoavara.net,resources=edgetransformations/status,verbs=get;update;patch
...
func (r *EdgeTransformationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx).WithValues("edgetransformation", req.NamespacedName)

	var et routeprismv1alpha1.EdgeTransformation
	...
	conf, err := renderNginxConf(&et, routingKey, dnsIP, defaultIP, defaultPort, variants, r.OperatorAPIAddr)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *EdgeTransformationReconciler) findCRForTarget(ctx context.Context, ns, target string) (*routeprismv1alpha1.ContextRoute, error) {
...
func (r *EdgeTransformationReconciler) collectVariantBackends(ctx context.Context, cr *routeprismv1alpha1.ContextRoute, targetName string) ([]VariantBackend, error) {
...
func (r *EdgeTransformationReconciler) firstReadyEndpoint(ctx context.Context, ns, svcName string) (string, int32, error) {
...
func (r *EdgeTransformationReconciler) lookupClusterDNSIP(ctx context.Context) (string, error) {
...
func (r *EdgeTransformationReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	mapHTTPRouteToET := func(_ context.Context, obj client.Object) []reconcile.Request {
		l := obj.GetLabels()
		if l[LabelManagedBy] != ManagedByValue || l[LabelKind] != KindEdgeTransformation {
			return nil
		}
		owner := l[LabelOwner]
		if owner == "" {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&routeprismv1alpha1.EdgeTransformation{}).
