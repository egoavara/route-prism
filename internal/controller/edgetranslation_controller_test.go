...
var _ = Describe("EdgeTranslation Controller", func() {
	const ns = "default"
	...
	cleanup := func() {
		_ = k8sClient.DeleteAllOf(ctx, &routeprismv1alpha1.EdgeTranslation{}, client.InNamespace(ns))
		...
		Eventually(func() int {
			var l routeprismv1alpha1.EdgeTranslationList
	...
	It("provisions translator + applies HTTPRoute on the target Service", func() {
		...
		r := &EdgeTranslationReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}
	...
	It("rejects unimplemented modes via status condition", func() {
		...
		var got routeprismv1alpha1.EdgeTranslation
