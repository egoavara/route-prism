...
func main() {
	...
	if err := (&controller.ContextRouteReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "ContextRoute")
		os.Exit(1)
	}
	if err := (&controller.EdgeTranslationReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("edgetranslation-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "EdgeTranslation")
		os.Exit(1)
	}
	// +kubebuilder:scaffold:builder

	// Startup preflight: verify the cluster's service mesh actually
	// accepts HTTPRoute parentRefs targeting a Service (Gateway API GAMMA).
	// Result is reported as a Kubernetes Event on this Pod itself, via
	// Downward API env vars set by config/manager/manager.yaml.
	...
	if err := mgr.Add(preflight.New(
		...
		setupLog.Error(err, "Failed to register preflight runner")
		os.Exit(1)
	}
